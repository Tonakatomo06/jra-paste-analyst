...
function parseJraOfficialText(text) {
  ...
  const starts = startsByEntry.length >= Math.max(1, Math.floor(entryNames.length * 0.7)) ? startsByEntry : findHorseStarts(lines);
...
function parseVanText(text) {
  ...
  const starts = startsByEntry.length >= Math.max(1, Math.floor(entryNames.length * 0.7)) ? startsByEntry : findVanHorseStarts(lines);
  const horses = starts.map((start, index) => {
    const end = starts[index + 1] || lines.length;
