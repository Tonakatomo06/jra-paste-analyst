...
function frameNoFromPostNo(postNo, fieldSize) {
  const no = Number(postNo);
  const size = Number(fieldSize);
  if (!no || !size) return null;
  if (size <= 8) return no;
  const singleFrames = Math.max(0, 16 - size);
  if (no <= singleFrames) return no;
  return Math.min(8, singleFrames + Math.ceil((no - singleFrames) / 2));
}

function attachEntrantNumbers(horses) {
  const fieldSize = horses.length;
  horses.forEach((horse, index) => {
    horse.postNo = horse.postNo || index + 1;
    horse.frameNo = horse.frameNo || frameNoFromPostNo(horse.postNo, fieldSize);
  });
  return horses;
}
...
function horseLabel(horse) {
  const prefix = [
    horse.frameNo ? `${horse.frameNo}枠` : "",
    horse.postNo ? `${horse.postNo}番` : "",
  ].filter(Boolean).join("");
  return prefix ? `${prefix} ${horse.name}` : horse.name;
}
...
function extractRaceTitle(lines) {
  return lines.find((line) => /\d+回.+\d+日\s+\d+R/.test(line))
    || lines.find((line) => /\d{4}\/\d+\/\d+.+\d+R/.test(line))
    || "レース名未取得";
}

function extractCurrentCourse(lines) {
  const top = lines.slice(0, 45);
  const joined = top.join(" ");
  const venue = top.find((line) => /^(札幌|函館|福島|新潟|東京|中山|中京|京都|阪神|小倉)$/.test(line))
    || firstMatch(joined, /\d+回(札幌|函館|福島|新潟|東京|中山|中京|京都|阪神|小倉)\d+日/);
  const courseLine = top.find((line) => /[芝ダ障][右左内外]*\s*\d{3,4}m/.test(line)) || "";
  const surface = firstMatch(courseLine, /(芝|ダ|障)/);
  const meters = firstMatch(courseLine, /(\d{3,4})m/);
  return venue && surface && meters ? `${venue} ${meters}${surface}` : "";
}

function mergeStarts(primary, fallback, expectedCount = 0) {
  const merged = [...primary];
  fallback.forEach((start) => {
    if (!merged.some((item) => Math.abs(item - start) <= 1)) merged.push(start);
  });
  const sorted = [...new Set(merged)].sort((a, b) => a - b);
  return expectedCount && sorted.length > expectedCount ? sorted.slice(0, expectedCount) : sorted;
}
...
function isHorseNameCandidate(line) {
  ...
  if (/^(ブリンカー着用|チークピーシズ着用|シャドーロール着用|メンコ着用)$/.test(text)) return false;
...
function findHorseStarts(lines) {
  ...
  for (let i = 0; i < lines.length - 3; i += 1) {
    if (/^枠.+/.test(lines[i]) && /^\d+$/.test(lines[i + 1])) {
...
function extractEntryNames(lines) {
  const basicIndex = lines.findIndex((x) => x === "基本");
  const end = basicIndex >= 0 ? basicIndex : lines.length;
  const names = [];
  for (let i = 0; i < end - 4; i += 1) {
    if (!/^\d{1,2}$/.test(lines[i])) continue;
    let j = i + 1;
    while (j < end && !isHorseNameCandidate(lines[j])) j += 1;
    if (j < end) names.push(lines[j]);
  }
  return [...new Set(names)];
}

function normalizeHorseKey(name) {
  return String(name || "").replace(/^(マルチ|地|外|□地|□外)+/, "").trim();
}

function findStartsByEntryNames(lines, names) {
  if (!names.length) return [];
  const basicIndex = lines.findIndex((x) => x === "基本");
  const startFrom = basicIndex >= 0 ? basicIndex : 0;
  const starts = [];
  const used = new Set();
  for (const name of names) {
    const key = normalizeHorseKey(name);
    for (let i = startFrom; i < lines.length; i += 1) {
      if (used.has(i)) continue;
      if (isHorseNameCandidate(lines[i]) && normalizeHorseKey(lines[i]) === key && lines.slice(i, i + 20).some((x) => /(美浦|栗東|地方)[)）]/.test(x))) {
        starts.push(i);
        used.add(i);
        break;
      }
...
function parseHorse(block) {
  ...
  const horse = {
    ...
    sexAgeColor: block.find((x) => /^[牡牝セ騙せん]\d+\/.+/.test(x)) || "",
    currentWeight: "",
    currentJockey: "",
    sire: "",
    dam: "",
...
function parseJraOfficialText(text) {
  ...
  const entryNames = extractEntryNames(lines);
  const startsByEntry = findStartsByEntryNames(lines, entryNames);
  const fallbackStarts = findHorseStarts(lines);
  const starts = startsByEntry.length
    ? mergeStarts(startsByEntry, fallbackStarts, entryNames.length)
    : fallbackStarts;
  const horses = starts.map((start, index) => {
    const end = starts[index + 1] || lines.length;
    return parseHorse(lines.slice(start, end));
  }).filter((horse) => horse.name);
  attachEntrantNumbers(horses);

  const runs = horses.flatMap((horse) => horse.runs.map((run) => ({ ...run, horse: horseLabel(horse) })));
  ...
  return {
    ...
    currentCourse: extractCurrentCourse(lines),
...
function findVanHorseStarts(lines) {
  const starts = [];
  for (let i = 0; i < lines.length - 12; i += 1) {
    if (!/^\d{1,2}$/.test(lines[i])) continue;
    const window = lines.slice(i, i + 18);
    if (!window.some((x) => /^\d+人気$/.test(x))) continue;
    if (!window.some((x) => /^\d+(?:\.\d+)?倍$/.test(x))) continue;
    if (!window.some((x) => /^父\s+/.test(x))) continue;
    starts.push(i);
  }
  return starts;
}

function extractVanEntryNames(lines) {
  const detailHeader = lines.findIndex((x) => x.includes("オッズ 馬名"));
  const end = detailHeader >= 0 ? detailHeader : lines.findIndex((x) => x === "基本");
  const limit = end >= 0 ? end : lines.length;
  const names = [];
  for (let i = 0; i < limit - 1; i += 1) {
    if (!/^\d{1,2}$/.test(lines[i])) continue;
    let j = i + 1;
    while (j < limit && !isHorseNameCandidate(lines[j])) j += 1;
    if (j < limit) names.push(lines[j]);
  }
  return [...new Set(names)];
}

function findVanStartsByEntryNames(lines, names) {
  if (!names.length) return [];
  const detailHeader = lines.findIndex((x) => x.includes("オッズ 馬名"));
  const startFrom = detailHeader >= 0 ? detailHeader : 0;
  const starts = [];
  const used = new Set();
  for (const name of names) {
    const key = normalizeHorseKey(name);
    for (let i = startFrom; i < lines.length; i += 1) {
      if (used.has(i)) continue;
      if (!/^\d{1,2}$/.test(lines[i])) continue;
      const window = lines.slice(i, i + 28);
      if (!window.some((x) => normalizeHorseKey(x) === key)) continue;
      if (!window.some((x) => /^父\s+/.test(x))) continue;
      starts.push(i);
      used.add(i);
      break;
...
function parseVanRun(segment) {
  const dateVenue = segment.find((x) => /(札幌|函館|福島|新潟|東京|中山|中京|京都|阪神|小倉)\d{2}\/\d{2}\/\d{2}/.test(x)) || "";
  const dateMatch = dateVenue.match(/(札幌|函館|福島|新潟|東京|中山|中京|京都|阪神|小倉)(\d{2})\/(\d{2})\/(\d{2})/);
  const date = dateMatch ? `20${dateMatch[2]}年${Number(dateMatch[3])}月${Number(dateMatch[4])}日` : "";
  const venue = dateMatch ? dateMatch[1] : "";
  const fieldGatePop = dateVenue.match(/(\d+)頭\s+(\d+)番\s+(\d+)人/);
  const run = {
    date,
    venue,
    className: segment.find((x) => /未勝利|新馬|特別|1勝|2勝|3勝|オープン/.test(x)) || "",
    finish: segment[0] && /^\d+|消|除外|取消|中止/.test(segment[0]) ? (segment[0] === "消" ? "取消" : `${segment[0]}着`) : "",
    fieldAndGate: fieldGatePop ? `${fieldGatePop[1]}頭 ${fieldGatePop[2]}番` : "",
    popularityText: fieldGatePop ? `${fieldGatePop[3]}番人気` : "",
    jockey: "",
    carriedWeight: "",
    distance: "",
    time: "",
  ...
  const raceLineIndex = segment.findIndex((x) => /(芝|ダ|障)\d{3,4}/.test(x));
  if (raceLineIndex >= 0) {
    const raceLine = segment[raceLineIndex];
    const distanceMatch = raceLine.match(/(芝|ダ|障)(\d{3,4})/);
    run.distance = distanceMatch ? `${distanceMatch[2]}${distanceMatch[1]}` : "";
    run.className = raceLine.replace(/\s*(芝|ダ|障)\d{3,4}.*/, "").trim() || run.className;
    const jockeyLine = segment[raceLineIndex + 1] || "";
    const jockeyMatch = jockeyLine.match(/^(.+?)\s+(\d{2}\.0)\s+(\d{3})kg$/);
    if (jockeyMatch) {
      run.jockey = jockeyMatch[1].trim();
      run.carriedWeight = `${jockeyMatch[2]}kg`;
      run.bodyWeightText = `${jockeyMatch[3]}kg`;
    }
    const timeLine = segment[raceLineIndex + 2] || "";
    const timeMatch = timeLine.match(/(\d+:\d{2}\.\d)\((\d+\.\d)\)/);
    if (timeMatch) {
      run.time = timeMatch[1];
      run.final3f = `3F ${timeMatch[2]}`;
    }
    const winnerLine = segment[raceLineIndex + 3] || "";
    const winnerMatch = winnerLine.match(/(.+)\(\+?(-?\d+(?:\.\d+)?)\)/);
    run.winnerText = winnerMatch ? `${winnerMatch[1].trim()}(${winnerMatch[2]})` : winnerLine;
  }
...
function parseVanHorse(block) {
  const oddsIndex = block.findIndex((x) => /^\d+(?:\.\d+)?倍$/.test(x));
  let name = "";
  if (oddsIndex >= 0) {
    for (let i = oddsIndex + 1; i < Math.min(block.length, oddsIndex + 6); i += 1) {
      if (isHorseNameCandidate(block[i])) {
        name = block[i];
        break;
      }
    }
  }
  const sire = (block.find((x) => /^父\s+/.test(x)) || "").replace(/^父\s+/, "");
  const dam = (block.find((x) => /^母\s+/.test(x)) || "").replace(/^母\s+/, "");
  const damsire = (block.find((x) => /^\(/.test(x) && !/\d+\.\d+\.\d+\.\d+/.test(x)) || "").replace(/[()（）]/g, "").trim();
  const sexAgeColor = block.find((x) => /^[牡牝セ騙せん]\d+\s+/.test(x)) || "";
  const bodyLine = block.find((x) => /^\[.*\]\s*\d{3}kg/.test(x)) || "";
  const currentWeight = firstMatch(bodyLine, /(\d{3}kg)/);
  const currentJockeyIndex = block.findIndex((x) => /^\d{2}\.0$/.test(x));
  const currentJockey = currentJockeyIndex > 0 ? block[currentJockeyIndex - 1] : "";
  const trainerIndex = block.findIndex((x) => /^\[.+\]/.test(x));
  const trainer = trainerIndex > 0 ? block[trainerIndex - 1] : "";
  const stable = firstMatch(block[trainerIndex] || "", /^\[(.+)\]/);
  const horse = {
    name,
    trainer,
    stable,
  ...
  const runStarts = [];
  for (let i = 0; i < block.length; i += 1) {
    if ((/^\d+$/.test(block[i]) || /^(消|除外|取消|中止)$/.test(block[i])) && block.slice(i, i + 8).some((x) => /(札幌|函館|福島|新潟|東京|中山|中京|京都|阪神|小倉)\d{2}\/\d{2}\/\d{2}/.test(x))) {
      runStarts.push(i);
    }
  }
  runStarts.forEach((start, index) => {
    const end = runStarts[index + 1] || block.length;
    const run = parseVanRun(block.slice(start, end));
    if (run.date || run.distance || run.finish) horse.runs.push(run);
  });
...
function parseVanText(text) {
  const lines = normalizeLines(text);
  const entryNames = extractVanEntryNames(lines);
  const startsByEntry = findVanStartsByEntryNames(lines, entryNames);
  const fallbackStarts = findVanHorseStarts(lines);
  const starts = startsByEntry.length
    ? mergeStarts(startsByEntry, fallbackStarts, entryNames.length)
    : fallbackStarts;
  const horses = starts.map((start, index) => {
    const end = starts[index + 1] || lines.length;
    return parseVanHorse(lines.slice(start, end));
  }).filter((horse) => horse.name);
  attachEntrantNumbers(horses);
  const runs = horses.flatMap((horse) => horse.runs.map((run) => ({ ...run, horse: horseLabel(horse) })));
  const groups = new Map();
...
function parseJraText(text) {
  if (/DM|My注目馬|ﾚｰｽ間隔|脚質/.test(text)) {
    const van = parseVanText(text);
    if (van.horses.length) return van;
  }
  return parseJraOfficialText(text);
}
...
function renderSummary(data) {
  ...
  panels.summary.innerHTML = `<div class="cards">${data.horses.map((horse) => `
    <article class="horse-card">
      <strong>${escapeHtml(horseLabel(horse))}</strong>
...
function renderRanking(data) {
  ...
  panels.ranking.innerHTML = `<table>
    ...
    <tbody>${ranked.map((horse, index) => {
      ...
      return `<tr>
        <td>${index + 1}</td>
        <td>${escapeHtml(horseLabel(horse))}</td>
...
function followupCandidates(data) {
  ...
  data.horses.forEach((horse) => {
    horse.runs.forEach((run) => {
      ...
      item.sourceHorses.add(horseLabel(horse));
...
function renderRuns(data) {
  const rows = data.horses.flatMap((horse) => horse.runs.map((run) => ({ horse: horseLabel(horse), ...run })));
...
function analyze() {
  ...
  if (textChanged && !currentCourseKey().label) {
    setCourseFromKey(data.currentCourse || inferCourse(data));
  }
  render(data);
}
